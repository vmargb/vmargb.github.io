...
** Month one: not getting better

The first month was genuinely painful. I mean getting the installation to work smoothly was one thing, but having to fight with your configuration afterwards is another.

You can't just =apt install= or =brew install= something. You can't just download a binary and run it. You can't just follow a tutorial that says "put this in =/usr/local/bin=." Everything must be declared and configured in a =.nix=. The friction is exhausting.
...
The error messages from Nix are sometimes just impenetrable. When something goes wrong in a derivation, you sometimes get a stack trace that requires knowing the internals of the Nix evaluator to parse. This gets better as you learn more but it's not fun at all.
