...
Every few months someone rediscovers Obsidian and posts about it somewhere, and the cycle begins again. That was me once. I spent a weekend migrating all my notes(from google keep) into a vault, installed all the essential plugins, and felt genuinely productive for about three weeks or so...

I'll preface by saying, this isn't an "Obsidian is bad" post. Obsidian is genuinely good, if not one of the best pieces of software that has graced us. But after using Emacs and Org-mode seriously for the past year, there is just no going back(annoyingly)

The problem isn't the features. It's that fundamentally all we are doing is *editing text*, and when you enter that market, you've already lost. The job of text editing has already matured and been perfected since the editor wars. An electron app in the 21st century is simply never going to beat Emacs(or even vim)

** What Obsidian gets right

One thing I'll say is, the onboarding is essentially frictionless. You point it at a folder, and you have a note-taking app right off the bat. The graph view is beautiful and the plugin ecosystem is already exhaustive, not to mention defaults are sensible with zero configuration. If you're a normal person who wants to immediately take notes in a frictionless way, you might as well stop reading here, Obsidian is the right answer.
...
My notes are just files on disk. The idea that I should have to open a specific application to interact with them started to feel counter-intuitive. With Org files, every tool I already know works: =grep=, =sed=, =find=, =git diff=, anything. They're plain text. Not to mention, my existing workflow for editing text outside of notetaking is consistent, it's one unified workflow that operates the same everwhere, regardless of whether I'm writing code or I'm writing this blog.

#+BEGIN_QUOTE
The test I now apply to any tool: if the company shut down tomorrow, or the app stopped working, would I lose anything I can't recover? With Obsidian vaults on disk, the answer is no. But the habits and workflows built around the GUI, yes. That's a different kind of lock-in.
#+END_QUOTE
